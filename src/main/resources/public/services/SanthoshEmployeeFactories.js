app.factory('AddTasksFactory', function ($resource, $location) {
    return $resource(($location.protocol() + "://" + $location.host() + ":" + $location.port()) + '/employees', {}, {
    	save: { method: 'POST' }
    });
});

app.factory('EditTasksFactory', function ($resource, $location) {
    return $resource(($location.protocol() + "://" + $location.host() + ":" + $location.port()) + '/employees/edit/:id',{}, {
    	update: { method: 'PUT' }
    });
});

app.factory('DeleteTasksFactory', function ($resource, $location) {
    return $resource(($location.protocol() + "://" + $location.host() + ":" + $location.port()) + '/employees/:id',{}, {
